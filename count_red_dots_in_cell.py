...
from skimage.filters import threshold_otsu, threshold_yen, gaussian, threshold_local
...
def get_filtered_cell_props(
    ...
    blue_channel = gaussian(blue_channel, sigma=gaussian_sigma)
    ...
    for prop in cell_props:
        ...
        # Filter out cells that touch the image border or are too small
        if (
            min_row > 0
            and min_col > 0
...
def get_props_red(image: ndarray, min_red_int: float, gaussian_sigma: float) -> tuple:
    """
    Retrieve properties of red dots in the image.

    Args:
        image (ndarray): The input image containing cells.
        min_red_int (float): Minimum red intensity to consider for thresholding.
        gaussian_sigma (float): Sigma value for Gaussian smoothing to reduce noise.
    ...
    smoothed_image = gaussian(red_channel, sigma=gaussian_sigma)
...
def count_plot_reds_per_cells(
    filtered_cell_props: list,
    filtered_cell_labels: ndarray,
    props_red: list,
    max_red_size: int,
    ...
    for red_prop in props_red:
        for i, prop in enumerate(filtered_cell_props, start=1):
            ...
            # Check if the red dot is within the cell and the size is between a minmal and maximal size
            if (
                cell_mask[int(red_prop.centroid[0]), int(red_prop.centroid[1])]
                and red_prop.num_pixels > 2
                and red_prop.num_pixels <= max_red_size
            ):
...
def analyze_image(
    full_file_path: str,
    output_dir_path: str,
    blue_hole_threshold: int,
    blue_gaussian_sigma: float,
    min_blue_size: int,
    max_red_size: int,
    red_gaussian_sigma: float,
) -> list:
    """
    ...
    if full_file_path.endswith(".tif") and "_counted.tif" not in full_file_path:
        ...
        filtered_cell_props, filtered_cell_labels = get_filtered_cell_props(
            image, blue_hole_threshold, blue_gaussian_sigma, min_blue_size
        )
...
def new_or_update_csv(output_dir_path: str, all_table_rows: str) -> None:
    """
    ...
    if os.path.isdir(args.input):
        ...
        for file in os.listdir(input_dir_path):
            ...
            table_row = analyze_image(
                full_file_path,
                output_dir_path,
                args.blue_hole_threshold,
                args.blue_gaussian_sigma,
                args.min_cell_size,
                args.red_dot_max_size,
                args.red_gaussian_sigma,
            )
            if table_row:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-blue_bg_threshold",
        dest="blue_gaussian_sigma",
        default=2,
        type=float,
        help="Adjustment for blue background noise. Set between 0 (no adjustment) and approx. 3 (default: 2)",
        required=False,
    )
    ...
    parser.add_argument(
        "-red_max_size",
        dest="red_dot_max_size",
        default=20,
        type=int,
        help="Maximal size of a red dot to be considered a dot and not background noise (default: 20)",
        required=False,
    )

    parser.add_argument(
        "-blue_hole_threshold",
        dest="blue_hole_threshold",
        default=64,
        type=int,
        help="Maximal pixel of a hole to be filled (cell area). Minimal value 0 (default: 64)",
        required=False,
    )
